...
import { retrieve } from "src/redux/actions/riwayat_tubel";

import DaftarDialog from "./DaftarDialog";

function Riwayat_tubelTab() {
  const theme = useTheme();

  const [page, setPage] = useState(2);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [open, setOpen] = useState(false);
  const label = "Riwayat_tubel";
  ...
  const datas = useSelector((state) => state.riwayat_tubel);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_riwayat_tubel}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
...
export default Riwayat_tubelTab;
