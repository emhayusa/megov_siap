...
import { retrieve } from "src/redux/actions/riwayat_pangkat";

import DaftarDialog from "./DaftarDialog";

function Riwayat_pangkatTab() {
  const theme = useTheme();

  const [page, setPage] = useState(2);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [open, setOpen] = useState(false);
  const label = "Riwayat_pangkat";
  ...
  const datas = useSelector((state) => state.riwayat_pangkat);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_riwayat_pangkat}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
...
export default Riwayat_pangkatTab;
