...
import { retrieve_pensiun } from "src/redux/actions/pegawai";
...
function PegawaiTab() {
  ...
  const datas = useSelector((state) => state.pegawai);
  const { user: currentUser } = useSelector((state) => state.auth);

  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(retrieve_pensiun());
  }, []);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        ...
                        <TableCell>{data.pegawai_id}</TableCell>
