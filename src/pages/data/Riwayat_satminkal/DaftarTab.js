...
import { retrieve } from "src/redux/actions/riwayat_satminkal";

import DaftarDialog from "./DaftarDialog";

function Riwayat_satminkalTab() {
  const theme = useTheme();

  const [page, setPage] = useState(2);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [open, setOpen] = useState(false);
  const label = "Riwayat_satminkal";
  ...
  const datas = useSelector((state) => state.riwayat_satminkal);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_riwayat_satminkal}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
...
export default Riwayat_satminkalTab;
