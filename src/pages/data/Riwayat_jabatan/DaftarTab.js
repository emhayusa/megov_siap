...
import { retrieve } from "src/redux/actions/riwayat_jabatan";

import DaftarDialog from "./DaftarDialog";

function Riwayat_jabatanTab() {
  ...
  const datas = useSelector((state) => state.riwayat_jabatan);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_riwayat_jabatan}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
...
export default Riwayat_jabatanTab;
