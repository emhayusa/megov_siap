...
import { retrieve } from "src/redux/actions/level_wilayah";

import DaftarDialog from "./DaftarDialog";

function Level_wilayahTab() {
  const theme = useTheme();

  const [page, setPage] = useState(2);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [open, setOpen] = useState(false);
  const label = "Level_wilayah";
  ...
  const datas = useSelector((state) => state.level_wilayah);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_level_wilayah}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
...
export default Level_wilayahTab;
