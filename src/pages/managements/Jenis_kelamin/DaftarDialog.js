...
import { create, remove, update } from "src/redux/actions/jenis_kelamin";
...
function Jenis_kelaminDialog(props) {
  ...
  function isValidated() {
    //console.log(user);
    return data.nama_jenis_kelamin.length > 0;
  }

  const save = (e) => {
    e.preventDefault();

    const { nama_jenis_kelamin } = data;
    if (isValidated()) {
      setLoading(true);
      dispatch(create(nama_jenis_kelamin))
        .then((data) => {
          setLoading(false);
    ...
    >
      ...
      <DialogContent>
        ...
        <TextField
          ...
          required
          fullWidth
          id="nama_jenis_kelamin"
          label="Nama Jenis_kelamin"
          name="nama_jenis_kelamin"
          value={data.nama_jenis_kelamin}
          onChange={handleInputChange}
          autoComplete="nama_jenis_kelamin"
          autoFocus
...
Jenis_kelaminDialog.propTypes = {
  onClose: PropTypes.func.isRequired,
  open: PropTypes.bool.isRequired,
  config: PropTypes.object.isRequired,
};

export default Jenis_kelaminDialog;
