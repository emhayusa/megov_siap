...
import { retrieve } from "src/redux/actions/jenis_kelamin";

import DaftarDialog from "./DaftarDialog";

function Jenis_kelaminTab() {
  const theme = useTheme();

  const [page, setPage] = useState(2);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [open, setOpen] = useState(false);
  const label = "Jenis_kelamin";
  ...
  const datas = useSelector((state) => state.jenis_kelamin);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_jenis_kelamin}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
...
export default Jenis_kelaminTab;
