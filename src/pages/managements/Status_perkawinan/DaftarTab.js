...
import { retrieve } from "src/redux/actions/status_perkawinan";

import DaftarDialog from "./DaftarDialog";

function Status_perkawinanTab() {
  const theme = useTheme();

  const [page, setPage] = useState(2);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [open, setOpen] = useState(false);
  const label = "Status_perkawinan";
  ...
  const datas = useSelector((state) => state.status_perkawinan);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_status_perkawinan}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
...
export default Status_perkawinanTab;
