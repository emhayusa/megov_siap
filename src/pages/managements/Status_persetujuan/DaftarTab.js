...
import { retrieve } from "src/redux/actions/status_persetujuan";

import DaftarDialog from "./DaftarDialog";

function Status_persetujuanTab() {
  const theme = useTheme();

  const [page, setPage] = useState(2);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [open, setOpen] = useState(false);
  const label = "Status_persetujuan";
  ...
  const datas = useSelector((state) => state.status_persetujuan);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_status_persetujuan}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
...
export default Status_persetujuanTab;
