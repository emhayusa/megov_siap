...
import { retrieve } from "src/redux/actions/perangkat";
...
function PerangkatTab() {
  ...
  const datas = useSelector((state) => state.perangkat);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_perangkat}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
