...
import { create, remove, update } from "src/redux/actions/perangkat";
...
function PerangkatDialog(props) {
  ...
  function isValidated() {
    //console.log(user);
    return data.nama_perangkat.length > 0;
  }

  const save = (e) => {
    e.preventDefault();

    const { nama_perangkat } = data;
    if (isValidated()) {
      setLoading(true);
      dispatch(create(nama_perangkat))
        .then((data) => {
          setLoading(false);
