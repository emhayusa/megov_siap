...
import { create, remove, update } from "src/redux/actions/basemap";
...
function BasemapDialog(props) {
  ...
  function isValidated() {
    //console.log(user);
    return data.nama_basemap.length > 0;
  }

  const save = (e) => {
    e.preventDefault();

    const { nama_basemap } = data;
    if (isValidated()) {
      setLoading(true);
      dispatch(create(nama_basemap))
        .then((data) => {
          setLoading(false);
    ...
    >
      ...
      <DialogContent>
        ...
        <TextField
          ...
          label="Nama Basemap"
          name="nama_basemap"
          value={data.nama_basemap}
          onChange={handleInputChange}
          autoComplete="nama_basemap"
          autoFocus
