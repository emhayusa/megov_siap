...
import { retrieve } from "src/redux/actions/pola_kerja";

import DaftarDialog from "./DaftarDialog";

function Pola_kerjaTab() {
  const theme = useTheme();

  const [page, setPage] = useState(2);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [open, setOpen] = useState(false);
  const label = "Pola_kerja";
  ...
  const datas = useSelector((state) => state.pola_kerja);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_pola_kerja}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
