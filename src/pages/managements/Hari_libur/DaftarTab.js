...
import { retrieve } from "src/redux/actions/hari_libur";

import DaftarDialog from "./DaftarDialog";

function Hari_liburTab() {
  const theme = useTheme();

  const [page, setPage] = useState(2);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [open, setOpen] = useState(false);
  const label = "Hari_libur";
  ...
  const datas = useSelector((state) => state.hari_libur);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_hari_libur}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
