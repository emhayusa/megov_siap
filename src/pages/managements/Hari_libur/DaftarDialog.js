...
import { create, remove, update } from "src/redux/actions/hari_libur";
...
function Hari_liburDialog(props) {
  ...
  function isValidated() {
    //console.log(user);
    return data.nama_hari_libur.length > 0;
  }

  const save = (e) => {
    e.preventDefault();

    const { nama_hari_libur } = data;
    if (isValidated()) {
      setLoading(true);
      dispatch(create(nama_hari_libur))
        .then((data) => {
          setLoading(false);
    ...
    >
      ...
      <DialogContent>
        ...
        <TextField
          ...
          fullWidth
          id="nama_hari_libur"
          label="Nama Hari_libur"
          name="nama_hari_libur"
          value={data.nama_hari_libur}
          onChange={handleInputChange}
          autoComplete="nama_hari_libur"
          autoFocus
...
Hari_liburDialog.propTypes = {
  onClose: PropTypes.func.isRequired,
  open: PropTypes.bool.isRequired,
  config: PropTypes.object.isRequired,
};

export default Hari_liburDialog;
