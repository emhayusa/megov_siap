...
import { retrieve } from "src/redux/actions/jenis_aktifitas";

import DaftarDialog from "./DaftarDialog";

function Jenis_aktifitasTab() {
  const theme = useTheme();

  const [page, setPage] = useState(2);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [open, setOpen] = useState(false);
  const label = "Jenis_aktifitas";
  ...
  const datas = useSelector((state) => state.jenis_aktifitas);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_jenis_aktifitas}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
...
export default Jenis_aktifitasTab;
