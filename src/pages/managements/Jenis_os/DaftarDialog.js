...
import { create, remove, update } from "src/redux/actions/jenis_os";
...
function Jenis_osDialog(props) {
  ...
  function isValidated() {
    //console.log(user);
    return data.nama_jenis_os.length > 0;
  }

  const save = (e) => {
    e.preventDefault();

    const { nama_jenis_os } = data;
    if (isValidated()) {
      setLoading(true);
      dispatch(create(nama_jenis_os))
        .then((data) => {
          setLoading(false);
    ...
    >
      ...
      <DialogContent>
        ...
        <TextField
          ...
          label="Nama Jenis_os"
          name="nama_jenis_os"
          value={data.nama_jenis_os}
          onChange={handleInputChange}
          autoComplete="nama_jenis_os"
          autoFocus
...
Jenis_osDialog.propTypes = {
  onClose: PropTypes.func.isRequired,
  open: PropTypes.bool.isRequired,
  config: PropTypes.object.isRequired,
};

export default Jenis_osDialog;
