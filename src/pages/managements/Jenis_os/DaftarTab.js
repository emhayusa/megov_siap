...
import { retrieve } from "src/redux/actions/jenis_os";

import DaftarDialog from "./DaftarDialog";

function Jenis_osTab() {
  const theme = useTheme();

  const [page, setPage] = useState(2);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [open, setOpen] = useState(false);
  const label = "Jenis_os";
  ...
  const datas = useSelector((state) => state.jenis_os);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_jenis_os}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
