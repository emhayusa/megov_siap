...
import { create, remove, update } from "src/redux/actions/wilayah";
...
function WilayahDialog(props) {
  ...
  function isValidated() {
    //console.log(user);
    return data.nama_wilayah.length > 0;
  }

  const save = (e) => {
    e.preventDefault();

    const { nama_wilayah } = data;
    if (isValidated()) {
      setLoading(true);
      dispatch(create(nama_wilayah))
        .then((data) => {
          setLoading(false);
...
WilayahDialog.propTypes = {
  onClose: PropTypes.func.isRequired,
  open: PropTypes.bool.isRequired,
  config: PropTypes.object.isRequired,
};

export default WilayahDialog;
