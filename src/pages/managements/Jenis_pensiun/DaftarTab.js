...
import { retrieve } from "src/redux/actions/jenis_pensiun";

import DaftarDialog from "./DaftarDialog";

function Jenis_pensiunTab() {
  const theme = useTheme();

  const [page, setPage] = useState(2);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [open, setOpen] = useState(false);
  const label = "Jenis_pensiun";
  ...
  const datas = useSelector((state) => state.jenis_pensiun);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_jenis_pensiun}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
...
export default Jenis_pensiunTab;
