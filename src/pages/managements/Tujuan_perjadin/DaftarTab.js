...
import { retrieve } from "src/redux/actions/tujuan_perjadin";

import DaftarDialog from "./DaftarDialog";

function Tujuan_perjadinTab() {
  const theme = useTheme();

  const [page, setPage] = useState(2);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [open, setOpen] = useState(false);
  const label = "Tujuan_perjadin";
  ...
  const datas = useSelector((state) => state.tujuan_perjadin);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_tujuan_perjadin}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
...
export default Tujuan_perjadinTab;
