...
import { retrieve } from "src/redux/actions/kedudukan_pegawai";

import DaftarDialog from "./DaftarDialog";

function Kedudukan_pegawaiTab() {
  const theme = useTheme();

  const [page, setPage] = useState(2);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [open, setOpen] = useState(false);
  const label = "Kedudukan_pegawai";
  ...
  const datas = useSelector((state) => state.kedudukan_pegawai);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_kedudukan_pegawai}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
...
export default Kedudukan_pegawaiTab;
