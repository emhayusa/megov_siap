...
import { retrieve } from "src/redux/actions/jenis_alat_angkut";

import DaftarDialog from "./DaftarDialog";

function Jenis_alat_angkutTab() {
  const theme = useTheme();

  const [page, setPage] = useState(2);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [open, setOpen] = useState(false);
  const label = "Jenis_alat_angkut";
  ...
  const datas = useSelector((state) => state.jenis_alat_angkut);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_jenis_alat_angkut}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
...
export default Jenis_alat_angkutTab;
