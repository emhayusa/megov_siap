...
import { retrieve } from "src/redux/actions/jenis_kp";

import DaftarDialog from "./DaftarDialog";

function Jenis_kpTab() {
  const theme = useTheme();

  const [page, setPage] = useState(2);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [open, setOpen] = useState(false);
  const label = "Jenis_kp";
  ...
  const datas = useSelector((state) => state.jenis_kp);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_jenis_kp}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
