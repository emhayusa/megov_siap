...
import { create, remove, update } from "src/redux/actions/jenis_kp";
...
function Jenis_kpDialog(props) {
  ...
  function isValidated() {
    //console.log(user);
    return data.nama_jenis_kp.length > 0;
  }

  const save = (e) => {
    e.preventDefault();

    const { nama_jenis_kp } = data;
    if (isValidated()) {
      setLoading(true);
      dispatch(create(nama_jenis_kp))
        .then((data) => {
          setLoading(false);
    ...
    >
      ...
      <DialogContent>
        ...
        <TextField
          ...
          label="Nama Jenis_kp"
          name="nama_jenis_kp"
          value={data.nama_jenis_kp}
          onChange={handleInputChange}
          autoComplete="nama_jenis_kp"
          autoFocus
...
Jenis_kpDialog.propTypes = {
  onClose: PropTypes.func.isRequired,
  open: PropTypes.bool.isRequired,
  config: PropTypes.object.isRequired,
};

export default Jenis_kpDialog;
