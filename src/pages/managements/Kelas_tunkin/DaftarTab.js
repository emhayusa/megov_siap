...
import { retrieve } from "src/redux/actions/kelas_tunkin";

import DaftarDialog from "./DaftarDialog";

function Kelas_tunkinTab() {
  const theme = useTheme();

  const [page, setPage] = useState(2);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [open, setOpen] = useState(false);
  const label = "Kelas_tunkin";
  ...
  const datas = useSelector((state) => state.kelas_tunkin);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_kelas_tunkin}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
...
export default Kelas_tunkinTab;
