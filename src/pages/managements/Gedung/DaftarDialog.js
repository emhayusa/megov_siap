...
import { create, remove, update } from "src/redux/actions/gedung";
...
function GedungDialog(props) {
  ...
  function isValidated() {
    //console.log(user);
    return data.nama_gedung.length > 0;
  }

  const save = (e) => {
    e.preventDefault();

    const { nama_gedung } = data;
    if (isValidated()) {
      setLoading(true);
      dispatch(create(nama_gedung))
        .then((data) => {
          setLoading(false);
...
GedungDialog.propTypes = {
  onClose: PropTypes.func.isRequired,
  open: PropTypes.bool.isRequired,
  config: PropTypes.object.isRequired,
};

export default GedungDialog;
