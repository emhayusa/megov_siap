...
import { create, remove, update } from "src/redux/actions/organisasi";
...
function OrganisasiDialog(props) {
  ...
  function isValidated() {
    //console.log(user);
    return data.nama_organisasi.length > 0;
  }

  const save = (e) => {
    e.preventDefault();

    const { nama_organisasi } = data;
    if (isValidated()) {
      setLoading(true);
      dispatch(create(nama_organisasi))
        .then((data) => {
          setLoading(false);
...
OrganisasiDialog.propTypes = {
  onClose: PropTypes.func.isRequired,
  open: PropTypes.bool.isRequired,
  config: PropTypes.object.isRequired,
};

export default OrganisasiDialog;
