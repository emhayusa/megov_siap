...
import { retrieve } from "src/redux/actions/organisasi";
...
function OrganisasiTab() {
  ...
  const datas = useSelector((state) => state.organisasi);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_organisasi}</TableCell>
                        <TableCell>{data.singkatan}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
