...
import { retrieve } from "src/redux/actions/kategori_proporsi";

import DaftarDialog from "./DaftarDialog";

function Kategori_proporsiTab() {
  const theme = useTheme();

  const [page, setPage] = useState(2);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [open, setOpen] = useState(false);
  const label = "Kategori_proporsi";
  ...
  const datas = useSelector((state) => state.kategori_proporsi);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_kategori_proporsi}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
...
export default Kategori_proporsiTab;
