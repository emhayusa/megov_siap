...
import { retrieve } from "src/redux/actions/tingkat_biaya_perjadin";

import DaftarDialog from "./DaftarDialog";

function Tingkat_biaya_perjadinTab() {
  const theme = useTheme();

  const [page, setPage] = useState(2);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [open, setOpen] = useState(false);
  const label = "Tingkat_biaya_perjadin";
  ...
  const datas = useSelector((state) => state.tingkat_biaya_perjadin);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_tingkat_biaya_perjadin}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
...
export default Tingkat_biaya_perjadinTab;
