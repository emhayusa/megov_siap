...
import { retrieve } from "src/redux/actions/status_pegawai";

import DaftarDialog from "./DaftarDialog";

function Status_pegawaiTab() {
  const theme = useTheme();

  const [page, setPage] = useState(2);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [open, setOpen] = useState(false);
  const label = "Status_pegawai";
  ...
  const datas = useSelector((state) => state.status_pegawai);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_status_pegawai}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
...
export default Status_pegawaiTab;
