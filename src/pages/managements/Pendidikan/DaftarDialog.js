...
import { create, remove, update } from "src/redux/actions/pendidikan";
...
function PendidikanDialog(props) {
  ...
  function isValidated() {
    //console.log(user);
    return data.nama_pendidikan.length > 0;
  }

  const save = (e) => {
    e.preventDefault();

    const { nama_pendidikan } = data;
    if (isValidated()) {
      setLoading(true);
      dispatch(create(nama_pendidikan))
        .then((data) => {
          setLoading(false);
...
PendidikanDialog.propTypes = {
  onClose: PropTypes.func.isRequired,
  open: PropTypes.bool.isRequired,
  config: PropTypes.object.isRequired,
};

export default PendidikanDialog;
