...
import { retrieve } from "src/redux/actions/status_kehadiran";

import DaftarDialog from "./DaftarDialog";

function Status_kehadiranTab() {
  const theme = useTheme();

  const [page, setPage] = useState(2);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [open, setOpen] = useState(false);
  const label = "Status_kehadiran";
  ...
  const datas = useSelector((state) => state.status_kehadiran);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_status_kehadiran}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
...
export default Status_kehadiranTab;
