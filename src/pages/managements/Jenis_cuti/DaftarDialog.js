...
import { create, remove, update } from "src/redux/actions/jenis_cuti";
...
function Jenis_cutiDialog(props) {
  ...
  function isValidated() {
    //console.log(user);
    return data.nama_jenis_cuti.length > 0;
  }

  const save = (e) => {
    e.preventDefault();

    const { nama_jenis_cuti } = data;
    if (isValidated()) {
      setLoading(true);
      dispatch(create(nama_jenis_cuti))
        .then((data) => {
          setLoading(false);
    ...
    >
      ...
      <DialogContent>
        ...
        <TextField
          ...
          fullWidth
          id="nama_jenis_cuti"
          label="Nama Jenis_cuti"
          name="nama_jenis_cuti"
          value={data.nama_jenis_cuti}
          onChange={handleInputChange}
          autoComplete="nama_jenis_cuti"
          autoFocus
...
Jenis_cutiDialog.propTypes = {
  onClose: PropTypes.func.isRequired,
  open: PropTypes.bool.isRequired,
  config: PropTypes.object.isRequired,
};

export default Jenis_cutiDialog;
