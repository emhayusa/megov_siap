...
import { retrieve } from "src/redux/actions/jenis_cuti";

import DaftarDialog from "./DaftarDialog";

function Jenis_cutiTab() {
  const theme = useTheme();

  const [page, setPage] = useState(2);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [open, setOpen] = useState(false);
  const label = "Jenis_cuti";
  ...
  const datas = useSelector((state) => state.jenis_cuti);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_jenis_cuti}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
