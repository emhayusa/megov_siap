...
import { create, remove, update } from "src/redux/actions/agama";
...
function AgamaDialog(props) {
  ...
  function isValidated() {
    //console.log(user);
    return data.nama_agama.length > 0;
  }

  const save = (e) => {
    e.preventDefault();

    const { nama_agama } = data;
    if (isValidated()) {
      setLoading(true);
      dispatch(create(nama_agama))
        .then((data) => {
          setLoading(false);
...
AgamaDialog.propTypes = {
  onClose: PropTypes.func.isRequired,
  open: PropTypes.bool.isRequired,
  config: PropTypes.object.isRequired,
};

export default AgamaDialog;
