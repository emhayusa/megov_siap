...
import { retrieve } from "src/redux/actions/jenis_jabatan";

import DaftarDialog from "./DaftarDialog";

function Jenis_jabatanTab() {
  const theme = useTheme();

  const [page, setPage] = useState(2);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [open, setOpen] = useState(false);
  const label = "Jenis_jabatan";
  ...
  const datas = useSelector((state) => state.jenis_jabatan);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_jenis_jabatan}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
...
export default Jenis_jabatanTab;
