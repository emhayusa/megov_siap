...
import { create, remove, update } from "src/redux/actions/kantor";
...
function KantorDialog(props) {
  ...
  function isValidated() {
    //console.log(user);
    return data.nama_kantor.length > 0;
  }

  const save = (e) => {
    e.preventDefault();

    const { nama_kantor } = data;
    if (isValidated()) {
      setLoading(true);
      dispatch(create(nama_kantor))
        .then((data) => {
          setLoading(false);
...
KantorDialog.propTypes = {
  onClose: PropTypes.func.isRequired,
  open: PropTypes.bool.isRequired,
  config: PropTypes.object.isRequired,
};

export default KantorDialog;
