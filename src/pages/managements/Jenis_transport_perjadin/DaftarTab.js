...
import { retrieve } from "src/redux/actions/jenis_transport_perjadin";

import DaftarDialog from "./DaftarDialog";

function Jenis_transport_perjadinTab() {
  const theme = useTheme();

  const [page, setPage] = useState(2);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [open, setOpen] = useState(false);
  const label = "Jenis_transport_perjadin";
  ...
  const datas = useSelector((state) => state.jenis_transport_perjadin);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_jenis_transport_perjadin}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
...
export default Jenis_transport_perjadinTab;
