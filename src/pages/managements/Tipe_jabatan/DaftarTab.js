...
import { retrieve } from "src/redux/actions/tipe_jabatan";

import DaftarDialog from "./DaftarDialog";

function Tipe_jabatanTab() {
  const theme = useTheme();

  const [page, setPage] = useState(2);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [open, setOpen] = useState(false);
  const label = "Tipe_jabatan";
  ...
  const datas = useSelector((state) => state.tipe_jabatan);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_tipe_jabatan}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
...
export default Tipe_jabatanTab;
