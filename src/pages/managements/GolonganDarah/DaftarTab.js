...
import { retrieve } from "src/redux/actions/golongan_darah";

import DaftarDialog from "./DaftarDialog";

function Golongan_darahTab() {
  const theme = useTheme();

  const [page, setPage] = useState(2);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [open, setOpen] = useState(false);
  const label = "Golongan_darah";
  ...
  const datas = useSelector((state) => state.golongan_darah);
                  ...
                  {datas &&
                    datas.map((data) => (
                      <TableRow key={data.uuid} hover>
                        <TableCell>{data.nama_golongan_darah}</TableCell>
                        {currentUser.roles.includes("ROLE_ADMIN") ? (
...
export default Golongan_darahTab;
