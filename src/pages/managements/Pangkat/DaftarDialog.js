...
import { create, remove, update } from "src/redux/actions/pangkat";
...
function PangkatDialog(props) {
  ...
  function isValidated() {
    //console.log(user);
    return data.nama_pangkat.length > 0;
  }

  const save = (e) => {
    e.preventDefault();

    const { nama_pangkat } = data;
    if (isValidated()) {
      setLoading(true);
      dispatch(create(nama_pangkat))
        .then((data) => {
          setLoading(false);
...
PangkatDialog.propTypes = {
  onClose: PropTypes.func.isRequired,
  open: PropTypes.bool.isRequired,
  config: PropTypes.object.isRequired,
};

export default PangkatDialog;
