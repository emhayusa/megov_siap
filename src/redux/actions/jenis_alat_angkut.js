...
import Service from "src/services/jenis_alat_angkut.service";
...
export const retrieve = () => async (dispatch) => {
  try {
    ...
    dispatch({
      type: RETRIEVE_JENIS_ALAT_ANGKUT_SUCCESS,
      payload: res.data,
    });
  } catch (err) {
...
export const create = (nama_jenis_alat_angkut) => async (dispatch) => {
  try {
    ...
    dispatch({
      type: CREATE_JENIS_ALAT_ANGKUT_SUCCESS,
      payload: res.data,
    });
...
export const update = (uuid, data) => async (dispatch) => {
  try {
    ...
    dispatch({
      type: UPDATE_JENIS_ALAT_ANGKUT_SUCCESS,
      payload: data,
    });
...
export const remove = (uuid) => async (dispatch) => {
  try {
    ...
    dispatch({
      type: DELETE_JENIS_ALAT_ANGKUT_SUCCESS,
      payload: { uuid },
    });
  } catch (err) {
    if (err.response && err.response.status === 401) {
