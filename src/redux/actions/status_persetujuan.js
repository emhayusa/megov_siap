...
import Service from "src/services/status_persetujuan.service";
...
export const retrieve = () => async (dispatch) => {
  try {
    ...
    dispatch({
      type: RETRIEVE_STATUS_PERSETUJUAN_SUCCESS,
      payload: res.data,
    });
  } catch (err) {
...
export const create = (nama_status_persetujuan) => async (dispatch) => {
  try {
    ...
    dispatch({
      type: CREATE_STATUS_PERSETUJUAN_SUCCESS,
      payload: res.data,
    });
...
export const update = (uuid, data) => async (dispatch) => {
  try {
    ...
    dispatch({
      type: UPDATE_STATUS_PERSETUJUAN_SUCCESS,
      payload: data,
    });
...
export const remove = (uuid) => async (dispatch) => {
  try {
    ...
    dispatch({
      type: DELETE_STATUS_PERSETUJUAN_SUCCESS,
      payload: { uuid },
    });
  } catch (err) {
    if (err.response && err.response.status === 401) {
