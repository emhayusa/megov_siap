...
import Service from "src/services/riwayat_tubel.service";
...
export const retrieve = () => async (dispatch) => {
  try {
    ...
    dispatch({
      type: RETRIEVE_RIWAYAT_TUBEL_SUCCESS,
      payload: res.data,
    });
  } catch (err) {
...
export const create = (data) => async (dispatch) => {
  try {
    ...
    dispatch({
      type: CREATE_RIWAYAT_TUBEL_SUCCESS,
      payload: res.data,
    });
...
export const update = (uuid, data) => async (dispatch) => {
  try {
    ...
    dispatch({
      type: UPDATE_RIWAYAT_TUBEL_SUCCESS,
      payload: data,
    });
...
export const remove = (uuid) => async (dispatch) => {
  try {
    ...
    dispatch({
      type: DELETE_RIWAYAT_TUBEL_SUCCESS,
      payload: { uuid },
    });
  } catch (err) {
    if (err.response && err.response.status === 401) {
